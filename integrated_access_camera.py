...
# Entity configuration for multi-controller support
...
def sync_transactions():
    ...
    try:
        ...
        while idx < len(txns):
            ...
            for txn in batch:
                try:
                    # Use timestamp as document ID with all data as fields
                    timestamp = txn.get("timestamp", int(time.time()))
                    
                    db.collection("transactions").document(str(timestamp)).set(txn)
                    synced += 1
                    logging.info(f"Successfully synced transaction with timestamp {timestamp}: {txn.get('card_number', 'unknown')}")
...
def get_transactions():
    ...
    try:
        ...
        if db is not None and is_internet_available():
            try:
                # Query all transactions, ordered by timestamp, then sort by entity_id
                docs_iter = db.collection("transactions") \
                              .order_by("timestamp", direction=firestore.Query.DESCENDING) \
                              .limit(200).stream()  # Get more to allow for entity filtering
                docs = list(docs_iter)
                
                # Filter by entity_id and sort
                entity_transactions = []
                for doc in docs:
                    tx = doc.to_dict() or {}
                ...
                # Sort by entity_id, then by timestamp
                entity_transactions.sort(key=lambda x: (x.get("entity_id", ""), x.get("timestamp", 0)), reverse=True)
                docs = entity_transactions[:50]  # Limit to 50 for display
            ...
            for tx in docs:
                transactions.append({
                    ...
                    "entity_id": tx.get("entity_id", "Unknown")
                })
        ...
        def on_snapshot(col_snapshot, changes, read_time):
            try:
                ...
                for change in changes:
                    ...
                    # Only process users for this entity
                    ...
                    if change.type.name in ("ADDED", "MODIFIED"):
                        ...
                        changed = True
                        logging.info(f"User {doc.get('name', 'Unknown')} (Card: {card_number}) added/updated for entity {ENTITY_ID}.")
                    elif change.type.name == "REMOVED":
                        if card_number in local:
                            local.pop(card_number, None)
                            changed = True
                            logging.info(f"User with Card {card_number} removed from entity {ENTITY_ID}.")
                    ...
                    if "blocked" in doc:
                        if doc["blocked"]:
                            if not local.get(card_number):
                                ...
                                changed = True
                                logging.info(f"User {card_number} blocked via Firebase for entity {ENTITY_ID}.")
                        else:
                            if local.pop(card_number, None) is not None:
                                changed = True
                                logging.info(f"User {card_number} unblocked via Firebase for entity {ENTITY_ID}.")
                if changed:
        ...
        try:
            if is_internet_available() and db is not None:
                try:
                    ...
                    logging.info(f"Transaction uploaded with timestamp {timestamp}: {transaction}")
                except Exception as e:
